...
var __shouldComponentUpdate = function __shouldComponentUpdate(props, nextProps) {
  ...
  var customizer = function customizer(oldProp, newProp, key) {
    ...
    if (key === "imgixParams") {
      var _value = (0, _common.shallowEqual)(oldProp, newProp, function (a, b) {
        if (Array.isArray(a)) {
    ...
    if (key === "htmlAttributes") {
      var _value2 = (0, _common.shallowEqual)(oldProp, newProp);
    ...
    var value = (0, _common.shallowEqual)(oldProp, newProp);

    if (!value) {
      var isEqual = JSON.stringify(oldProp) === JSON.stringify(newProp);
  ...
  var propsEqual = (0, _common.shallowEqual)(cloneObject(props), cloneObject(nextProps), customizer);
  return !propsEqual;
};
